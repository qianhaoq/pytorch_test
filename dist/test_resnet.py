...
#model_SGD = resnet18().cuda()
#model_Momentum = resnet18().cuda()
#model_RMSprop = resnet18().cuda()
#model_Adam = resnet18().cuda()
#model_Adamax = resnet18().cuda()
...
fig1 = plt.figure("不同网络深度与损失值的关系")
ax1 = fig1.add_subplot(111)
for e in range(len(optimizers)):
    ax1.plot(x, loss_plot[e], label=opt_name[e], linestyle=line_name[e])
plt.title("不同网络深度与损失值的关系")
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend()
plt.savefig("不同网络深度与损失值的关系.png")  
plt. close(0)

fig2 = plt.figure("不同网络深度与准确率的关系")
ax2 = fig2.add_subplot(111)
for e in range(len(optimizers)):
    ax2.plot(x, acc_plot[e], label=opt_name[e], linestyle=line_name[e])
plt.title("不同网络深度与准确率的关系")
plt.ylabel('Acc')
plt.xlabel('epoch')
plt.legend()
plt.savefig("不同网络深度与准确率的关系.png")  
plt. close(0)
# plt.show()
